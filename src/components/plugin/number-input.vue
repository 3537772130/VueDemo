<template>
  <el-input :placeholder="placeholder" @blur="onBlurHandler" @input="onInputHandler"
    @focus="onFocusHandler" ref="numeric" type="tel" v-model="amount"
...
<script>
    ...
    export default {
        ...
        created (){
            let sellPrice = this.$cookies.get('goods_price')
            if (sellPrice){
                this.$cookies.remove('goods_price')
                this.init(sellPrice)
            } else {
                this.init(0.00)
            }
