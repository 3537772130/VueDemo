  ...
  <el-container>
    <el-main v-loading="loading" element-loading-text="加载中" style="background-color: #FFFFFF;">
      <div style="width: 100%;float: left;clear: left;margin: auto;">
        <el-form :inline="true" :model="typeForm" :rules="rules" ref="typeForm" class="demo-form-inline goods-type-form">
          <el-form-item label="类型图标" prop="typeLogo">
            <div class="goods-type-input" style="display: inline-block;text-align: left;">
              <el-upload
                ...
                <img v-if="typeForm.typeLogo" :src="typeForm.typeLogo + timestamp" class="type-logo">
                <i v-else class="el-icon-plus type-logo-uploader-icon"></i>
              </el-upload>
            </div>
            <el-input v-model="typeForm.typeLogo" class="goods-type-input" style="display: none;"></el-input>
          </el-form-item>
          <el-form-item label="类型名称" prop="typeName">
            <el-input v-model="typeForm.typeName" placeholder="请输入类型名称"
                      class="goods-type-input"></el-input>
          </el-form-item>
          <el-form-item label="营业状态" prop="typeStatus">
            <el-select v-model="typeForm.typeStatus" class="goods-type-input">
              <el-option label="正常" value='1'></el-option>
              <el-option label="禁用" value='0'></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label=" ">
            <el-button type="success" @click="onSubmit()" class="goods-type-input" style="text-align: center;">提交
            </el-button>
          </el-form-item>
        </el-form>
      </div>
    ...
    methods: {
      loadGoodsType(typeId) {
        if (typeId && typeId != '0') {
          this.loading = true
          this.$axios({
            url: '/api/user/goods/queryGoodsType',
            method: 'post',
            data: {id: typeId}
          }).then(res => {
